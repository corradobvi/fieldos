...
router.get("/allenamenti", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const selectFields = isGenitore
      ? "a.id, a.titolo, a.obiettivo, a.data, a.durata_totale_minuti, COUNT(als.id) AS num_sessioni"
    ...
    const join = isGenitore
      ? "LEFT JOIN allenamento_sessioni als ON als.allenamento_id = a.id"
      : `LEFT JOIN allenamento_sessioni als ON als.allenamento_id = a.id
...
router.post("/allenamenti/:id/sessioni", requireAuth, requirePermission("modifica_piano_allenamento"), async (req, res) => {
  const { userId, societyId } = req.jwtUser!;
  const { id }                = req.params;
  const { sessione_libreria_id, titolo, descrizione, durata_minuti, categoria, tag, ordine } =
    req.body as Record<string, any>;
...
router.patch("/allenamenti/:id/sessioni/:sessioneId", requireAuth, requirePermission("modifica_piano_allenamento"), async (req, res) => {
  ...
  try {
    ...
    const { titolo_snapshot, descrizione_snapshot, durata_minuti_snapshot, categoria_snapshot, tag_snapshot, ordine } =
      req.body as Record<string, any>;
    const updates: string[] = [];
    const params: any[]     = [];
