...
router.get("/quote", requireAuth, requireRole("admin", "dirigente"), async (req, res) => {
...
// POST /api/v2/quote
router.post("/quote", requireAuth, requireRole("admin", "dirigente"), async (req, res) => {
  const { societyId } = req.jwtUser!;
...
// PUT /api/v2/quote/:id
router.put("/quote/:id", requireAuth, requireRole("admin", "dirigente"), async (req, res) => {
  const { societyId } = req.jwtUser!;
...
router.delete("/quote/:id", requireAuth, requireRole("admin", "dirigente"), async (req, res) => {
  const { societyId } = req.jwtUser!;
  try {
