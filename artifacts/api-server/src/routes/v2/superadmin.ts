...
// TEMPORARY: migrate-users-to-existing — FASE 2bis 2026-05-22 — remove after use
router.post("/superadmin/migrate-users-to-existing", async (req, res) => {
  if (req.headers["x-sa-secret"] !== SA_SECRET) return res.status(401).json({ error: "unauthorized" });
  ...
  const [dupRows] = (await pool.execute(
    `SELECT id, email, society_id FROM users WHERE LOWER(email) IN (${ph})`, emails
  )) as [any[], any];
  if (dupRows.length > 0) {
    return res.status(409).json({
      error: "duplicate_emails",
      found: dupRows.map((r: any) => ({ id: r.id, email: r.email, society_id: r.society_id })),
    });
  }

  // Leggi blob per ogni utente e recupera plain password
  const usersReady: Array<{ u: typeof users[0]; plainPass: string }> = [];
  for (const u of users) {
    const [blobRows] = (await pool.execute(
      "SELECT state_json FROM society_state WHERE `key` = ? LIMIT 1", [u.sourceBlob]
    )) as [any[], any];
    if (!blobRows.length) return res.status(404).json({ error: "source_blob_not_found", sourceBlob: u.sourceBlob });
    let state: any;
    try { state = JSON.parse(blobRows[0].state_json as string); } catch {
      return res.status(500).json({ error: "blob_parse_error", sourceBlob: u.sourceBlob });
    }
    const blobUser = (state.USERS_DB || []).find(
      (b: any) => typeof b.email === "string" && b.email.toLowerCase() === u.email.toLowerCase()
    );
  ...
  try {
    ...
    for (const { u, plainPass } of usersReady) {
      ...
      const [uRes] = (await conn.execute(
        ...
        [u.societyId, u.nome, u.cognome, u.email.toLowerCase(), pwHash,
         u.ruolo, u.leva ?? null, u.phone ?? null]
      )) as [any, any];
