...
// GET /api/v2/superadmin/ai-budget-diagnose — TEMPORANEO, rimuovere dopo diagnosi
router.get("/superadmin/ai-budget-diagnose", async (req, res) => {
  if (req.headers["x-sa-secret"] !== SA_SECRET) return res.status(401).json({ error: "unauthorized" });

  try {
    const mese = new Date().toISOString().slice(0, 7); // 'YYYY-MM'

    // Query 1: tutte le righe budget con info joined
    ...
    // Query 2: righe duplicate (stesso mister_id o societa_id, stesso mese)
    ...
    // Query 3: ultimi 30 log AI con info utente/società
    ...
    // Query 4: stessi userId in più società
